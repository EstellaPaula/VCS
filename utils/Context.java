    ...
    public void run() {
        ...
        while (true) {
            ...
            if (!wasError && this.isTrackable(operation)) {
                ...
                String comand;
                comand = String.valueOf(operation.getType());
                ArrayList<String> operationArgs
                        = new ArrayList<>(Arrays.asList(comand.split("\\s+")));
                if (String.valueOf(operation.getType()).equals("TOUCH")) {
                    String s;
                    s = "\tCreated file " + operation.getOperationArgs().get(1) + "\n";
                    vcs.getCommand().add(s);
                }
                if (String.valueOf(operation.getType()).equals("MAKEDIR")) {
                    String s;
                    s = "\tCreated directory " + operation.getOperationArgs().get(1) + "\n";
                    vcs.getCommand().add(s);
                }
                if (String.valueOf(operation.getType()).equals("CHANGEDIR")) {
                    String s;
                    s = "\tChanged directory to " + operation.getOperationArgs().get(0) + "\n";
                    vcs.getCommand().add(s);
                }
                if (String.valueOf(operation.getType()).equals("REMOVE")) {
                    String s;
                    s = "\tRemoved " + operation.getOperationArgs().get(0) + "\n";
                    vcs.getCommand().add(s);
                }
                if (String.valueOf(operation.getType()).equals("WRITETOFILE")) {
                    String s;
                    s = "\tAdded \"" + operation.getOperationArgs().get(1)
                            + "\" to file " + operation.getOperationArgs().get(0) + "\n";
                    vcs.getCommand().add(s);
                }
            }
        }
